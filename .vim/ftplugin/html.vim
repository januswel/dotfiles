...
if !(exists('no_plugin_maps') && no_plugin_maps)
            \ && !(exists('no_fthtml_maps') && no_example_maps)

    " hasmapto() and <unique> are required to avoid overlap
    if !hasmapto('<Plug>InsertHtmlCloseTag', 'i')
        imap <unique><buffer><C-b>
                    \ <Plug>InsertHtmlCloseTag
    endif
...
inoremap <script><silent><buffer><Plug>InsertHtmlCloseTag
            \ <Esc>:call <SID>InsertHtmlCloseTag()<CR>a
" modify by HTML Tidy
...
if !exists('*s:InsertHtmlCloseTag')
    function s:InsertHtmlCloseTag()
